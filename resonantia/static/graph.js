...
export async function renderSessionGraph(elementId, payload, callbacks) {
  ...
  cy.on("tap", "node", (evt) => {
    ...
    if (nodeType === "sttp") {
      ...
      if (typeof callbacks?.onGraphNodeSelected === "function") {
        callbacks.onGraphNodeSelected(sessionId, nodeIndex);
      }
      return;
    }
    ...
    const sessionId = evt.target.data("sessionId");
    if (!sessionId) return;
    if (typeof callbacks?.onGraphSessionSelected === "function") {
      callbacks.onGraphSessionSelected(sessionId);
    }
  });
